...
void lcd_send(uint8_t value, uint8_t mode) {
	if (mode) {
		LCD_RS_PORT = LCD_RS_PORT | (LCD_RS);
		} else {
		LCD_RS_PORT = LCD_RS_PORT & ~(LCD_RS);
	}

	LCD_RW_PORT = LCD_RW_PORT & ~(LCD_RW);
...
void lcd_write_nibble(uint8_t nibble) {
	//Need to set each bit separately
	//LCD_PORT = (LCD_PORT & 0xff & ~(0x0f << LCD_D0)) | ((nibble & 0x0f) << LCD_D0);
	//
	//All bits D4 - D7 has to be set by 4 bits from 4 bits of nibble 
	//number = (number & ~(1UL << n)) | (x << n);
	LCD_DB4_PORT = (LCD_DB4_PORT & ~(LCD_DB4)) | (nibble << PC6);
	LCD_DB5_PORT = (LCD_DB5_PORT & ~(LCD_DB5)) | (nibble << PC7);
	LCD_DB6_PORT = (LCD_DB6_PORT & ~(LCD_DB6)) | (nibble << PA6);
	LCD_DB7_PORT = (LCD_DB7_PORT & ~(LCD_DB7)) | (nibble << PA5);

	LCD_E_PORT = LCD_E_PORT & ~(LCD_E); //clear bit
	LCD_E_PORT = LCD_E_PORT | (LCD_E);  //set bit
	LCD_E_PORT = LCD_E_PORT & ~(LCD_E); //clear bit
