...
namespace nm_encoder{
	...
	encoder::encoder(){
		...
		input_poll::get_object()->register_event(GUID_ENC, ENC1_IN_UP);
		input_poll::get_object()->register_event(GUID_ENC, ENC1_IN_DOWN);
	...
	*/
	...
	uint8_t encoder::get_encoder_state(){
		...
		uint8_t _state_ = (((ENC_PORT>>(ENC_PORT_A-1))&2U)|  //check second bit (and 00000010)
		   		      ((ENC_PORT>>ENC_PORT_B)&1U));     // check last bit
		
		return(_state_);
	}
	
	uint8_t encoder::get_button_state(){
		
		uint8_t _state_ = (ENC_PORT>>ENC_PORT_SW)&1U;  //0 or 1
